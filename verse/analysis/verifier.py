...
class Verifier:
    ...
    def compute_full_reachtube_step(
        ...
        for agent_id in node.agent:
            ...
            if agent_id not in node.trace:
                ...
                if consts.reachability_method == "DRYVR":
                    ...
                    raise ValueError(f"Reachability computation method {consts.reachability_method} not available.")
                # num_calls += 1
        ...
        if old_node_id != None:
            if old_node_id[0] != consts.run_num:
                ...
                new_cache, paths_to_sim = to_simulate(old_node.agent, node.agent, cached_trans_tubes)
                # pp(("to sim", new_cache.keys(), len(paths_to_sim)))

        # Get all possible transitions to next mode
        asserts, all_possible_transitions = Verifier.get_transition_verify_opt(new_cache, paths_to_sim, node, consts.lane_map, consts.sensor)
        ...
        if asserts != None:
            ...
            return (node.id, later, next_nodes, node.trace, asserts, cache_tube_updates, cache_trans_tube_updates)    
    ...
    def proc_result(self, id, later, next_nodes, traces, assert_hits, cache_tube_updates, cache_trans_tube_updates, max_height):
        ...
        for new, aid, transit_agents, transition, transition_idx, run_num in cache_trans_tube_updates:
            cached = self.trans_cache.check_hit(aid, done_node.mode[aid], combine_all(done_node.init[aid]), done_node.init)
            if new and not cached:
                self.trans_cache.add_tube(aid, combined_inits, done_node, transit_agents, transition, transition_idx, run_num)
                self.num_cached += 1
            else:
                assert cached != None
                cached.transitions.extend(convert_reach_trans(aid, transit_agents, done_node.init, transition, transition_idx))
    ...
    def compute_full_reachtube(
        ...
        num_calls = 0
        num_transitions = 0
        consts = ReachConsts(time_step, lane_map, init_seg_length, reachability_method, run_num, past_runs, sensor, agent_dict)
    ...
    @staticmethod
    def apply_reset(agent: BaseAgent, reset_list, all_agent_state, track_map) -> Tuple[str, np.ndarray]:
        dest = []
        rect = []
